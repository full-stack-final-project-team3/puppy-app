...
const TreatsListForDog = () => {
  ...
  useEffect(() => {
    const fetchTreatsList = async () => {
      try {
        const response = await fetch(`${TREATS_URL}/list/${dogId}?pageNo=${pageNo}&sort=${sort}`, { // API 호출
          method: "GET",
          headers: {
  ...
  return (
    <div className={styles.treatsList}>
      <div className={styles.content}>
        <h1>{dogName ? `${dogName}의` : '강아지'} 간식 리스트</h1> {/* 강아지 이름 표시 */}
        {treatsList.length === 0 ? (
          <p>등록된 간식이 없습니다.</p>
        ) : (
          <ul className={styles.treatList}>
            {treatsList.map((treat) => (
              <li className={styles.treat} key={treat.id}>{treat.title}</li> // 각 간식 이름 표시
            ))}
          </ul>
        )}
      </div>
